...
async def transfer_call(room_name: str, trunk_id: str, transfer_to: str, **kwargs):
    """
    ...
    try:
        ...
        agent_participant = get_agent_participant(
        ...
        response = await sip_service.transfer_call(
            room_name=room_name, trunk_id=trunk_id, transfer_to=transfer_to
        )

        await sip_service.remove_participant(
            room_name=room_name, identity=agent_participant["identity"]
        )
        ...
        await llm.push_frame(
            TTSSpeakFrame(
                "It seems there was some issue transfering the call. You can try again or I continue conversing with you."
            )
        )
...
inhouse_tools = {"transfer_call": transfer_call}

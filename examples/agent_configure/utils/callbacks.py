...
custom_callbacks.register_callback(
    AgentEvent.CLIENT_CONNECTED, on_client_connected_callback
)

custom_callbacks.register_callback(
    AgentEvent.FIRST_PARTICIPANT_JOINED, on_first_participant_joined_callback
)

custom_callbacks.register_callback(
    AgentEvent.PARTICIPANT_LEFT, on_participant_left_callback
)

custom_callbacks.register_callback(
    AgentEvent.TRANSCRIPT_UPDATE, on_transcript_update_callback
)

custom_callbacks.register_callback(
    AgentEvent.CLIENT_DISCONNECTED, on_participant_left_callback
)
